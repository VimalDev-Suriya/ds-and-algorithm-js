...
const get_position_in_sorted_array = (arr, target) => {
    ...
    while (start <= end) {
        ...
        if (arr[start] <= arr[mid]) {
            ...
            // * Here I used if/else not multiple else, because I am 100% sure that if one half is not sorted, then another half is definetly sorted, beause we are sure it is ROTATED SORTED array, so if we rotate a sorted array for n number of times, there will always have only one pivot point, which determins the sorted half
